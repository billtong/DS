...
public:
	void solve(vector<vector<char>>& board) {
		...
		for (int first = 0, last = 0; first < colSize; first++, last++) {	//check the first and last row
			if (board[0][first] == 'O') 
		...
		for (int first = 0, last = 0; first < rowSize; first++, last++) {	//check the first and last column
			if (board[first][0] == 'O') 

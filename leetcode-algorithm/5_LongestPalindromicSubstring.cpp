...
public:
	...
	string longestPalindrome(string s)
	{
		...
		for (int i = n - 1; i >= 0; i--)
		{
			for (int j = i; j < n; j++)
			{
				...
				if (start < 0 || (table[i][j] && j - i + 1 > max_len))  //record the pos of palindrome substring
				{
					start = i;
					max_len = j - i + 1;
				}
	...
	*/
	string longestPalindrome1(string s)
	{
		...
		for (int i = 0; i < m; i++)
		{
			int len1 = expandAroundCenter(s, i, i);		//search "cac" type
			int len2 = expandAroundCenter(s, i, i + 1);	//search "caac" type
